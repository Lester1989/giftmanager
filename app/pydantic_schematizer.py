...
def create_pydantic(namespace:dict=None,suffix:str='API',default_type:type=str):
    ...
    def create_pydantic_inner(original_class):
        def _filter_for_no_default(n,val):
            return not n.startswith('_') and not isinstance(val,property) and not isinstance(val,function) and ('default' not in val.__dict__ or not val.default)

        def _filter_for_default(n,val):
            return not n.startswith('_') and not isinstance(val,property) and not isinstance(val,function) and (val.primary_key or ('default'  in val.__dict__ and val.default))
